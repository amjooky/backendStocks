...
// Removed tenant middleware - not needed
...
router.get('/', [
    ...
    try {
        ...
        // Add agency isolation
        const agencyFilter = addAgencyFilter(whereClause, params, req.agencyFilter, 'p');
        whereClause = agencyFilter.whereClause;
        let filteredParams = agencyFilter.params;

        // Build WHERE clause based on filters
        if (search) {
            whereClause += ' AND (p.name LIKE ? OR p.sku LIKE ? OR p.barcode LIKE ?)';
            const searchPattern = `%${search}%`;
            filteredParams.push(searchPattern, searchPattern, searchPattern);
        }

        if (categoryId) {
            whereClause += ' AND p.category_id = ?';
            filteredParams.push(categoryId);
        }

        if (supplierId) {
            whereClause += ' AND p.supplier_id = ?';
            filteredParams.push(supplierId);
        }
        ...
        const products = await getAllRows(query, filteredParams);
        ...
        const countQuery = `
            ...
            ${whereClause}
        `;
        const countParams = filteredParams.slice(0, -2); // Remove limit and offset
...
router.get('/:id', async (req, res) => {
    try {
        ...
        let whereClause = 'WHERE p.id = ? AND p.is_active = 1';
...
router.post('/', requireManager, [
    ...
    try {
        ...
        const queries = [
            {
                query: `INSERT INTO products (name, description, sku, barcode, category_id, supplier_id, cost_price, selling_price, min_stock_level, agency_id) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                params: [
                    name, description || null, sku, barcode || null, 
                    categoryId, supplierId || null, costPrice, sellingPrice, 
                    minStockLevel || 0, req.body.agencyId
                ]
            }
        ];
 ...
 *         description: Server error
 */
router.put('/:id', requireManager, validateResourceOwnership('products'), [
    body('name').notEmpty().withMessage('Product name is required'),
    body('sku').notEmpty().withMessage('SKU is required'),
    body('barcode').optional().isString(),
    body('categoryId').isInt().withMessage('Valid category ID is required'),
...
router.delete('/:id', requireManager, validateResourceOwnership('products'), async (req, res) => {
    try {
        const { id } = req.params;
