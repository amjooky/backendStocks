...
// Helper function to check and create stock notifications
async function checkAndCreateStockNotifications(productId, currentStock, userId) {
    try {
        // Get product details including minimum stock level
        ...
        if (currentStock === 0) {
            ...
            message = `🚨 CRITICAL: ${product.name} (${product.sku}) is OUT OF STOCK! Immediate restocking required.`;
            priority = 'high';
            shouldCreateNotification = true;
        } else if (currentStock <= minStock) {
            notificationType = 'stock_warning';
            title = 'Low Stock Warning';
            message = `⚠️ LOW STOCK: ${product.name} (${product.sku}) has ${currentStock} units left (minimum: ${minStock}). Consider restocking soon.`;
            priority = 'medium';
            shouldCreateNotification = true;
        }

        if (shouldCreateNotification) {
            // Check if there's already an active notification for this product within the last hour
            const existingNotification = await getRow(`
                SELECT id FROM notifications 
                WHERE type IN ('stock_alert', 'stock_warning')
                AND status = 'active'
            ...
            if (!existingNotification) {
                ...
                `, [
                    ...
                    JSON.stringify({
                        ...
                        stock_status: currentStock === 0 ? 'out_of_stock' : 'low_stock',
                        triggered_by: 'stock_operation'
                    })
                ]);

                console.log(`📢 Stock notification created: ${title} for ${product.name}`);
            }
        } else {
            // Stock is back to normal - mark any existing low stock notifications as resolved
            await runQuery(`
                UPDATE notifications 
                SET status = 'resolved', read_at = CURRENT_TIMESTAMP
                WHERE type IN ('stock_alert', 'stock_warning')
                AND status = 'active'
                AND JSON_EXTRACT(data, '$.product_id') = ?
            `, [productId]);
...
// Get all inventory items - root endpoint
...
router.post('/stock-in', requireManager, [
    ...
    try {
        ...
        // Check if we need to create/update stock notifications
        await checkAndCreateStockNotifications(productId, newStock, req.user.id);
...
router.post('/stock-out', requireManager, [
    ...
    try {
        ...
        // Check if we need to create stock notifications
        await checkAndCreateStockNotifications(productId, newStock, req.user.id);
