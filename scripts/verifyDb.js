...
const db = new sqlite3.Database(dbPath, (err) => {
    ...
    db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='users';", [], (err, row) => {
        ...
        // Check admin user
        db.get("SELECT id, username, email, role, is_active FROM users WHERE username = 'admin';", [], (err, user) => {
            if (err) {
            ...
            db.get("SELECT password FROM users WHERE username = 'admin';", [], (err, pwdRow) => {
                ...
                console.log('\nTo manually verify password, run:');
                console.log('node -e "require(\'bcryptjs\').compare(\'admin123\', \'' + pwdRow.password + '\').then(console.log)"');
